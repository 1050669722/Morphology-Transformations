...
from preprocessing.xml_h import *


def show_pic(img, bboxes=None, class_name=None):
    '''
    输入:
        img:图像array
    ...
    for i in range(len(bboxes)):
        ...
        x_max = bbox[2]
        y_max = bbox[3]
        cv2.rectangle(img, (int(x_min), int(y_min)), (int(x_max), int(y_max)), (r, g, b), 1)
        cv2.putText(img, classname, (int(x_min), int(y_min - 2)), fontFace=font, fontScale=0.8, thickness=1,
                    color=(r, g, b))
        test1 = cv2.rectangle(img, (int(x_min), int(y_min)), (int(x_max), int(y_max)), (r, g, b), 1)
    return test1
...
class DataAugmentForObjectDetection():
    def __init__(self, rotation_rate=0.2, max_rotation_angle=5,
                 crop_rate=0.5, shift_rate=0.6, change_light_rate=0.5,
    ...
    def _changeLight(self, img):
        ...
        if np.any(img < 0):
            ...
            flag = random.uniform(0.8, 1.8)  # flag>1为调暗,小于1为调亮
    ...
    def _rotate_img_bbox(self, img, bboxes, angle=5, scale=1.):
        ...
        for bbox in bboxes:
            ...
            # point5 = np.dot(rot_mat, np.array([xmin, ymin, 1]))
            # point6 = np.dot(rot_mat, np.array([xmax, ymax, 1]))
            # 合并np.array
            ...
            # rot_bboxes.append([int(point5[0]), int(point5[1]), int(point6[0]), int(point6[1])])
    ...
    def methods(self, num, img, bboxes):
        method = {
            0: self._crop_img_bboxes(img, bboxes),
            1: self._shift_pic_bboxes(img, bboxes),
            2: self._changeLight(img),
            3: self._addNoise(img),
            4: self._cutout(img, bboxes, length=self.cut_out_length, n_holes=self.cut_out_holes,
                            threshold=self.cut_out_threshold),
            5: self._filp_pic_bboxes(img, bboxes)
        }
        if num == 2 or num == 3 or num == 4:
            img = method[num]
        else:
            img, bboxes = method[num]
        return img, bboxes
    ...
    def dataAugment(self, img, bboxes):
        ...
        while change_num < 2:  # 默认至少有一种数据增强生效
            if random.random() < 0.4:  # 裁剪

                print('裁剪')
                change_num += 1
                img, bboxes = self._crop_img_bboxes(img, bboxes)

            # if random.random() > 0.8:  # 旋转
            #     print('旋转')
            #     change_num += 1
            ...
            if 0.2 < random.random() < 0.5:  # 平移
                print('平移')
                change_num += 1
                img, bboxes = self._shift_pic_bboxes(img, bboxes)

            if random.random() < 0.5:  # 改变亮度
                print('亮度')
                change_num += 1
                img = self._changeLight(img)

            if 0.45 < random.random() < 0.7:  # 加噪声
                print('加噪声')
                change_num += 1
                img = self._addNoise(img)

            if 0.5 < random.random() < 0.9:  # cutout
                print('cutout')
                change_num += 1
                img = self._cutout(img, bboxes, length=self.cut_out_length, n_holes=self.cut_out_holes,
                                   threshold=self.cut_out_threshold)

            if 0.3 < random.random() < 0.6:  # 翻转
                print('翻转')
                change_num += 1
...
if __name__ == '__main__':
    ...
    for parent, _, files in os.walk(source_pic_root_path):
        ...
        for file in files:
            j = 0
            while j < need_aug_num:
                pic_path = os.path.join(parent, file)
                xml_path = os.path.join(source_xml_root_path, file[:-4] + '.xml')

                coords = parse_xml(xml_path)  # 解析得到box信息，格式为[[x_min,y_min,x_max,y_max,name]]
                print(coords)
                classname = [coord[-1] for coord in coords]
                coords = [coord[:4] for coord in coords]
                print(coords)
                img = cv2.imread(pic_path)
                # show_pic(img, coords, classname)    # 原图
                if j == 0:
                    auged_img, auged_bboxes = dataAug._crop_img_bboxes(img, coords)
                    i += 1
                elif j == 1:
                    auged_img, auged_bboxes = dataAug._shift_pic_bboxes(img, coords)
                    i += 1
                elif j == 2:
                    auged_img = dataAug._changeLight(img)
                    auged_bboxes = coords
                    i += 1
                elif j == 3:
                    auged_img, auged_bboxes = dataAug._filp_pic_bboxes(img, coords)
                    i += 1
                elif j == 4:
                    auged_img = dataAug._addNoise(img)
                    auged_bboxes = coords
                    i += 1
                elif j == (need_aug_num - 1):
                    for angle in range(10, 360, 10):
                        scale = random.uniform(0.7, 0.8)
                        auged_img, auged_bboxes = dataAug._rotate_img_bbox(img, coords, angle, scale)
                        ...
                        for k in range(len(auged_bboxes)):
                            ...
                            f = open(txt_path, 'w')
                            f.writelines(str(auged_bboxes[k]) + '\n')
                            f.close()
                            anchor = open(es_anchor, 'a')
                ...
                aug_img = show_pic(auged_img, auged_bboxes, classname)  # 强化后的图
                cv2.imwrite(aug_name, aug_img)
                # raw_image = show_pic(img, coords, classname)
